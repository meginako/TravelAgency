                            ...
                            <div class="table-responsive">
                                ...
                                <table style="width: 100%;" id="example1" style="align-items: stretch;"
                                    ...
                                    <tbody>
                                        <?php
                                        //we are here ??????????????????????????????????????????????????????????????????????????????????????????????????
                                        $row = $conn->query("SELECT * FROM schedule where agency_id = 2 ORDER BY id DESC");
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                            Route : <select class="form-control" name="route_id" required id="">
                                ...
                                <?php
                                //we are here ??????????????????????????????????????????????????????????????????????????????????????????????????
                                $con = connect()->query("SELECT * FROM route where agency_id = 2 ");
                                while ($row = $con->fetch_assoc()) {
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            Vehicle : <select class="form-control" name="train_id" required id="">
                                ...
                                <?php
                                //we are here ??????????????????????????????????????????????????????????????????????????????????????????????????
                                $con = connect()->query("SELECT * FROM train where agency_id = ");
                                while ($row = $con->fetch_assoc()) {
                                    echo "<option value='" . $row['id'] . "'>" . $row['name'] . "</option>";
                                }
                                ?>
