                        ...
                        <div class="card-body">
                            ...
                            <table id="example1" style="align-items: stretch;"
                                ...
                                <tbody>
                                    <?php
                                    //we are here ???????????????????????????????????????????????
                                    $row = $conn->query("SELECT * FROM route where agency_id = '$agency_id' ");
...
if (isset($_POST['submit'])) {
    ...
    if (!isset($stop, $start)) {
        ...
        $conn = connect();
 //we are here ???????????????????????????????????????????????????????????????????
        $ins = $conn->prepare("INSERT INTO route (agency_id,start,stop) VALUES (?,?,?)");
        $ins->bind_param("sss",$agency_id, $start, $stop);
        $ins->execute();
        alert("Route Added!");
